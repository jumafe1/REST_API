...
from socialapi import BackgroundTasks
...
async def test_confirm_user_expired_token(async_client: AsyncClient, mocker):
    mocker.patch("socialapi.security.confirm_token_expires_minutes", return_value=-1)
...
async def test_user_does_not_exist(async_client: AsyncClient):
    response = await async_client.post(
        "/token", data={"username": "test@example.com", "password": "1234"}
    )
    assert response.status_code == 401
